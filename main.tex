...
To improve the efficiency, we will use "Fast relaxed clock model". 
\item Specify priors

Under the fast relaxed clock model, the mandatory prior distribution includes prior on evolutionary rates. 
\item Specify operators

To choose the ConstantDistance operator, we will need to set up some configurations. In the operators panel (click "View" -> "Show Operators panel"), we will see 
...
After running BEAST2, three files are generated under directory. 
\begin{enumerate}
